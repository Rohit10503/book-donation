...
$sql="select * from `users-books` where `serial no.` not in (select `serial no.` from `users-books` where `username`='$name' and `useremail`='$email');";
...
<head>
    ...
    <title>Wisdom Books 📚 -- others books</title>
    <link rel="stylesheet" href="\bookwebsite\pages\users-books\userbook.css">
</head>
<body>
        ...
        <?php
            if(isset($_GET['btn'])){
                session_start();
                $_SESSION['serial_no']=$_REQUEST['btn'];
    ...
    <div class="mainbody">
        <div class="head">
            <span id="head" class="head" >Let's Have a look on Others Books.</span>
        </div>
        <div class="bookfield">
        <div class="row">
            <?php
            ...
            for($i=0 ; $i<$num; $i++)
            {
                ...
                <div class="card" >
                  ...
                  <div class="texts">
                      ...
                      <div>
                        <span><span class="bold" >Donor Name : </span> '. $row['username'] . ' </span>
                      </div>
                      <button style="cursor:pointer;" class="a"   name="btn" value="'.$row['serial no.'] .'" type="submit" >View</button>
                     <!-- <input name="btn" class="a" type="submit" value="'.$row['serial no.'] .'" >-->
                      </div>
                </div>
              </div> 
            </form> 
